...
ALWAYS dispatch each task as a separate subagent with `isolation: "worktree"`. Do not implement them sequentially in the main session — the reason is the worktree isolation, not the parallelism: independent tasks editing one tree conflict. One agent per task, no more. Do not add extra agents to verify each other's work.
