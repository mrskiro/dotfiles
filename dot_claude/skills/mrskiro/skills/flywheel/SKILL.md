...
- `agent_id` / `parent_agent_id` on `claude_code.tool` spans — sub-agent nesting (default max depth 3, raise with `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`) and which agent spawned which
...
- `api_refusal`, `api_retries_exhausted`, model `effort` level (`low`/`medium`/`high`/`xhigh`/`max`), `claude-opus-5[1m]`-style 1M-context model variants — failure signals and model-mix analysis
- `claude_code.assistant_response` log events — emitted only when `OTEL_LOG_ASSISTANT_RESPONSES=1`; pairs with the existing prompt and tool-detail logs to close the request→response loop
